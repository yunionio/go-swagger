...
func TestSerializer_Methods(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Category", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			buf := bytes.NewBuffer(nil)
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("category.go", buf.Bytes())
				if assert.NoError(t, err) {
					res := string(ct)
...
func TestSerializer_Categories(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Categories", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("categories.go", buf.Bytes())
				if assert.NoError(t, err) {
					res := string(ct)
...
func TestSerializer_Product(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Product", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("product.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
					...
					assertInCode(t, "Categories Categories `json:\"categories\"`", res)
...
func TestSerializer_ProductLine(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("ProductLine", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("product_line.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_Scores(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Scores", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("scores.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_JaggedScores(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("JaggedScores", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("JaggedScores.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_Notables(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Notables", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
...
func TestSerializer_Notablix(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Notablix", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			assert.False(t, genModel.IsAnonymous)
...
func TestSerializer_WithComplexItems(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithComplexItems", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
...
func TestSerializer_WithItemsAndAdditional(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithItemsAndAdditional", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
...
func TestSerializer_WithItemsAndAdditional2(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithItemsAndAdditional2", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
...
func TestSerializer_WithComplexAdditional(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithComplexAdditional", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
...
func TestSerializer_Age(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Age", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
...
func TestSerializer_Flag(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("flag", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsPrimitive)
			assert.True(t, genModel.IsAliased)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("flag.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_FlagsList(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("flags_list", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			assert.True(t, genModel.IsAliased)
...
func TestSerializer_ImageTar(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("ImageTar", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
...
func TestSerializer_Tag(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Tag", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.True(t, genModel.IsAliased)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("tag.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
					...
					assertInCode(t, "func (m *Tag) MarkNameSet()", res)
					assertInCode(t, "func (m *Tag) MarkNameUnset()", res)
					assertInCode(t, "func (m *Tag) MarkNameNil()", res)
					assertInCode(t, "func (m *Tag) MarkNameZero()", res)
...
func TestSerializer_Stats(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Stats", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			assert.True(t, genModel.IsAliased)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("stats.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_Statix(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Statix", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsArray)
			assert.True(t, genModel.IsAliased)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("Statix.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_WithItems(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, err) {
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("with_items.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_Nota(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("Nota", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("nota.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_NotaWithRef(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("NotaWithRef", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.True(t, genModel.IsAliased)
			assert.True(t, genModel.IsMap)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("nota_with_ref.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_NotaWithMeta(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("NotaWithMeta", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.True(t, genModel.IsAliased)
			assert.True(t, genModel.IsMap)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("nota_with_meta.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_NotaWithName(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("NotaWithName", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
...
func TestSerializer_NotaWithRefRegistry(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("NotaWithRefRegistry", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.True(t, genModel.HasAdditionalProperties)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("nota_with_ref_registry.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_NotaWithMetaRegistry(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("NotaWithMetaRegistry", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.True(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithMap(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, err) {
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("with_map.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_WithMapRef(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithMapRef", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithMapComplex(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithMapComplex", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithMapRegistry(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithMapRegistry", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithMapRegistryRef(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithMapRegistryRef", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithMapComplexRegistry(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithMapComplexRegistry", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithAdditional(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithAdditional", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_SimpleTuple(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("SimpleTuple", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
			...
			err := templates.MustGet("model").Execute(buf, genModel)
			if assert.NoError(t, err) {
				ct, err := opts.LanguageOpts.FormatContent("with_tuple.go", buf.Bytes())
				// fmt.Println(buf.String())
				if assert.NoError(t, err) {
					res := string(ct)
					// fmt.Println(res)
...
func TestSerializer_TupleWithExtra(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("TupleWithExtra", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_TupleWithComplex(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("TupleWithComplex", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.False(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithTuple(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithTuple", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
...
func TestSerializer_WithTupleWithExtra(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		opts := uopts()
		genModel, err := makeGenDefinition("WithTupleWithExtra", "models", schema, specDoc, opts)
		if assert.NoError(t, err) {
			assert.True(t, genModel.IsComplexObject)
			assert.False(t, genModel.IsAdditionalProperties)
			assert.False(t, genModel.HasAdditionalProperties)
